<x-layout.dashboard title="Add Team Member" active="teams" contentClass="pr-4">
    <x-slot:header>
        <x-layout.page-header title="Add Team Member" :back="route('dashboard.teams.index')">
            <x-slot:subtitle>Create a new studio contributor profile</x-slot:subtitle>
            <x-slot:actions>
    ...
    <form id="team-form" method="POST" action="{{ route('dashboard.teams.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
