<x-layout.dashboard title="Journal Categories" active="journal-categories">
    <x-slot:header>
        <x-layout.page-header title="Journal Categories" :back="route('dashboard.jurnal.index')">
            <x-slot:subtitle>Organize article categories for your journal</x-slot:subtitle>
            <x-slot:actions>
                <button
                    type="button"
                    data-category-open="jurnal-category-dialog"
    ...
    <div class="space-y-8">
        @if ($categories->isEmpty())
            <div class="card-photography rounded-[2.5rem] p-16 text-center">
                ...
                <p class="text-sm text-zinc-500 mb-8">Create categories to organize and filter your articles.</p>
                <button
                    type="button"
                    data-category-open="jurnal-category-dialog"
                    data-category-mode="create"
                    ...
                    <article class="card-photography rounded-[2rem] p-6 flex flex-col gap-4">
                        ...
                        <div class="flex items-center gap-2 pt-2">
                            <button
                                ...
                                data-category-update-url="{{ route('dashboard.jurnal.category.update', $category) }}"
                                class="flex-1 text-center py-3 bg-white/5 rounded-xl text-xs font-black hover:bg-[#ff6b35] hover:text-white transition-all"
                            >
                                Edit
                            </button>
                            <form id="delete-jurnal-category-{{ $category->id }}" method="POST" action="{{ route('dashboard.jurnal.category.destroy', $category) }}" class="hidden">
                                @csrf
                                @method('DELETE')
                            </form>
                            <button
                                type="button"
                                data-alert-open="jurnal-category-delete-dialog"
                                data-alert-title="Delete {{ $category->title }}?"
                                data-alert-description="This category will be permanently removed. Articles using it must be reassigned first."
                                data-alert-confirm="Delete Category"
                                data-alert-form="delete-jurnal-category-{{ $category->id }}"
