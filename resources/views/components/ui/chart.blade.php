...
<div class="grid grid-cols-1 xl:grid-cols-4 gap-8">
    <div class="xl:col-span-3 card-photography p-6 lg:p-10 rounded-[3rem]">
        <div class="flex flex-wrap justify-between items-center gap-4 mb-10">
            <h2 class="text-2xl font-display font-black">{{ $title }}</h2>
            @isset($filters)
            ...
            @foreach ($bars as $index => $height)
                ...
                >
                    @if ($index === $highlightIndex)
                        <div class="absolute -top-10 left-1/2 -translate-x-1/2 bg-white text-black text-[10px] font-black px-2 py-1 rounded scale-0 group-hover:scale-100 transition-all whitespace-nowrap">
                            {{ $highlightLabel }}
                        </div>
                    @endif
                </div>
    ...
    <div class="card-photography p-6 lg:p-10 rounded-[3rem]" style="background: linear-gradient(135deg, rgba(245, 242, 237, 0.05) 0%, transparent 100%);">
        @isset($sidebar)
            {{ $sidebar }}
        @else
