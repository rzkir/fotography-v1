...
function renderFeatured(featuredSlot, article) {
    ...
    featuredSlot.innerHTML = `
        <a href="${article.url}" class="group block article-card" data-aos="fade-up">
            <div class="aspect-video overflow-hidden bg-zinc-900 mb-8 rounded-sm">
...
function renderPinned(pinnedColumn, pinnedItems, pinnedArticles) {
    ...
    pinnedItems.innerHTML = pinnedArticles
        .map(
            (article, index) => `
        <div class="article-pinned" data-aos="fade-up" data-aos-delay="${index * 100}">
...
function renderGrid(grid, gridArticles) {
    ...
    grid.innerHTML = gridArticles
        .map((article, index) => {
            ...
            return `
            <div class="article-grid ${offsetClass}" data-aos="fade-up" data-aos-delay="${(index % 6) * 80}">
    ...
    const getFilteredArticles = () => {
        return sortArticles(
            articles.filter((article) => {
                ...
                const matchesSearch =
                    !searchQuery || article.search.includes(searchQuery);
